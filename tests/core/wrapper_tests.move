...
fun package_witness_authorizes_wrapper_account_unwrap() {
    let mut scenario = ts::begin(@0xA);
    {
        ...
        asset_policy::authorize_witness<TEST, WrapperWitness>(&mut asset, &policy_cap);
...
fun allowlist_wrapper_deposit_and_unwrap_flow() {
    let mut scenario = ts::begin(@0xA);
    {
        ...
        kyc_admin::set_kyc(&mut asset, &policy_cap, user_identity, kyc::approved(), 0, b"user-kyc");
        kyc_admin::set_kyc(
            &mut asset,
            &policy_cap,
            wrapper_identity,
...
#[test, expected_failure(abort_code = 5, location = regulated_account::authorization)]
...
#[test, expected_failure(abort_code = 5, location = regulated_account::authorization)]
fun deauthorized_package_witness_cannot_transfer() {
    let mut scenario = ts::begin(@0xA);
    {
        let ctx = scenario.ctx();
        let (
        ...
        asset_policy::authorize_witness<TEST, WrapperWitness>(&mut asset, &policy_cap);
        assert!(asset::authorized_witness<TEST, WrapperWitness>(&asset), 0);
        asset_policy::deauthorize_witness<TEST, WrapperWitness>(&mut asset, &policy_cap);
        assert!(!asset::authorized_witness<TEST, WrapperWitness>(&asset), 1);
        ledger::mint(
            &mut asset,
            &mint_cap,
            authority::no_time(),
