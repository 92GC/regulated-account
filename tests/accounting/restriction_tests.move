...
use regulated_account::restricted_lots;
...
fun clawback_enforces_min_positive_balance_for_admin_recovery() {
    let mut scenario = ts::begin(@0xA);
    {
        let ctx = scenario.ctx();
        let (
...
fun restricted_mint_with_clock_prunes_expired_lots_before_capacity_check() {
    let mut scenario = ts::begin(@0xA);
    {
        ...
        let max_lots = constants::max_restricted_lots();
        ledger::mint(
            &mut asset,
            &mint_cap,
            authority::no_time(),
        ...
        while (i < max_lots) {
            ...
            external_ref_hash.push_back(((i / 256) as u8));
            external_ref_hash.push_back(((i % 256) as u8));
            account::add_restricted_lot(
                &mut alice,
                1,
                1,
                external_ref_hash,
        ...
        assert_eq!(account::restricted_lot_count(&alice), max_lots);
        assert_eq!(account::restricted_locked_balance_at(&alice, authority::no_time()), max_lots);
...
#[test, expected_failure(abort_code = 25, location = regulated_account::restricted_lots)]
fun restricted_lot_limit_rejects_new_active_lot_after_max() {
    let mut scenario = ts::begin(@0xA);
    {
        let ctx = scenario.ctx();
        let (
        ...
        assert_eq!(account::restricted_lot_count(&alice), max_lots);
        account::add_restricted_lot(&mut alice, 1, 10_000, b"overflow");
...
fun matching_restricted_lots_are_coalesced() {
    let mut scenario = ts::begin(@0xA);
    {
        ...
        let lots = account::restricted_lots(&alice);
        assert_eq!(lots.length(), 1);
        let lot = &lots[0];
        assert_eq!(restricted_lots::amount(lot), 100);
        assert_eq!(restricted_lots::unlock_ms(lot), 10_000);
        assert_eq!(restricted_lots::external_ref_hash(lot), b"same-grant");
