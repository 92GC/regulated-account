...
public fun mint_restricted<T>(
    ...
    mint_internal(asset, cap, to, amount, now_ms, &approvals, false);
...
/// Admin burn can debit frozen, locked, or otherwise non-transferable balance.
/// Pass `authority::clock_time` when the account may have restricted lots; `no_time`
/// only works for accounts with no active restricted-lot records.
...
/// Admin recovery transfer that can debit frozen, locked, or otherwise non-transferable balance.
/// Pass `authority::clock_time` when the debited account may have restricted lots; `no_time`
/// only works for accounts with no active restricted-lot records.
